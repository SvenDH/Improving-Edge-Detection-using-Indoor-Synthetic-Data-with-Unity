...
class BDCN(nn.Module):
    def __init__(self, pretrained=None, rate=4):
        super(BDCN, self).__init__()
        self.pretrained = pretrained
        t = 1
        """
        vgg = models.vgg16(pretrained=pretrained)
        self.features = nn.Sequential(*list(vgg.features.children())[0:30])
        self.features[4].ceil_mode=True
        self.features[9].ceil_mode = True
        self.features[16].ceil_mode = True
        self.features[23].ceil_mode = True
        self.features[23].stride = 1
        self.features[24].padding = (2,2)
        self.features[24].dilation = (2, 2)
        self.features[26].padding = (2, 2)
        self.features[26].dilation = (2, 2)
        self.features[28].padding = (2, 2)
        self.features[28].dilation = (2, 2)
        """
        self.features = VGG16_C()
    ...
    def _initialize_weights(self):
        for name, param in self.state_dict().items():
            if self.pretrained and 'features' in name:
                continue
